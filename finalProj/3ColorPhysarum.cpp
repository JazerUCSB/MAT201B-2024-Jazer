  ...
  void onCreate() override
  {
    ...
    if (isPrimary())
    {
      for (int i = 0; i < numParticles; i++)
      {
        ...
        state().colors[i] = randC;//Vec3f(randC.h, randC.s, randC.v);
    ...
    for (int i = 0; i < numParticles; i++)
    {
      ...
      //HSV col = HSV(state().colors[i].x, state().colors[i].y, state().colors[i].z);
      
      mesh.color(state().colors[i]);
